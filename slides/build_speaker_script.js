...
children.push(P("The strategy family is deliberately simple, and it has exactly two dials. The first is alpha, the smoothing — think of it as how long the strategy's MEMORY is. It keeps a running estimate of fair value built from past prices; small alpha means a long memory — a calm, stable anchor that barely reacts to any single day — while big alpha means it mostly remembers the last two or three days and chases the latest price. The second dial is delta, the entry threshold — how big a DISCOUNT the strategy demands before it buys. Small delta is a hair trigger: any two-cent dip below the estimate and it's in. Big delta is picky: it waits for a full ten-cent dip — roughly a two-sigma event at our noise level — before calling it a real mispricing rather than wiggle."));
children.push(P("One-liner to remember: alpha is how long the memory is; delta is how big a discount it demands before buying. And the two interact, because the dip is measured AGAINST the smoothed estimate: with fast smoothing the estimate hugs the price so tightly that a ten-cent gap almost never opens — which is why one corner of our grid basically never trades. We test the full three-by-three grid of alpha and delta, against two benchmarks: buy-and-hold — the “market is right” position — and never-trade, the sanity floor at exactly zero."));
...
children.push(P("The winner's character is patience on both dials: the longest memory in the grid, and the biggest discount demanded — it only acts on roughly two-sigma dips, the ones most likely to be genuine mispricing rather than wiggle. And notice the corner cell — short memory with a big discount demanded — that one almost never trades: a fast average hugs the price so closely that a ten-cent gap never opens. Memory and discount interact."));
